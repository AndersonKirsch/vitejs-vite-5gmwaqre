...
export function useDashboardResumo(
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<ResumoDashboard> => {
      ...
      const despGeraisRes = await supabase.from('despesas_gerais').select('imovel_id, competencia, valor, categoria, investimento').in('imovel_id', todosImovelIds).gte('competencia', inicio).lt('competencia', fimExclusivo);
...
export function useReceitaPorOrigem(unidadeIds: string[], inicio: string, fimExclusivo: string) {
  return useQuery({
    ...
    queryFn: async (): Promise<OrigemReceita[]> => {
      ...
      const rm = await supabase.from('receitas_manuais').select('valor_liquido, competencia, categoria').in('unidade_id', unidadeIds).gte('competencia', inicio).lt('competencia', fimExclusivo); for (const r of rm.data ?? []) { const cat = String((r as any).categoria || ''); const chave = cat === 'Diárias' ? 'Airbnb' : (cat.indexOf('direta') >= 0 || cat.indexOf('Aluguel') >= 0 ? 'Direta' : 'Outros'); porOrigem[chave] = (porOrigem[chave] ?? 0) + Number(r.valor_liquido ?? 0); } for (const r of data ?? [])
        porOrigem[r.origem] =
